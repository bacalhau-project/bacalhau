...
export class PipelineStack extends cdk.Stack {
          ...
          actions: [
            new codepipeline_actions.CloudFormationCreateUpdateStackAction({
              ...
              templatePath: cdkBuildOutput.atPath(
                "BacalhauCanaryStaging.template.json",
              ),
          ...
          actions: [
            ...
            new codepipeline_actions.ManualApprovalAction({
              ...
              additionalInformation:
                "Approve the Canary deployment to production",
            }),
          ],
        },
        {
          ...
          actions: [
            new codepipeline_actions.CloudFormationCreateUpdateStackAction({
              ...
              templatePath: cdkBuildOutput.atPath(
                "BacalhauCanaryProd.template.json",
              ),
            ...
            new codepipeline_actions.CloudFormationCreateUpdateStackAction({
              ...
              templatePath: cdkBuildOutput.atPath(
                "BacalhauCanaryProdOwned.template.json",
              ),
      ...
      buildSpec: codebuild.BuildSpec.fromObject({
        ...
        phases: {
          install: {
            commands: [
              "rm -rf `goenv root`",
              "curl --silent --show-error --location --fail https://go.dev/dl/go1.21.8.linux-amd64.tar.gz | tar --extract --gzip --file=- --directory=/usr/local",
        ...
        artifacts: {
          ...
          files: ["scenario_handler", "alarm_handler"],
        },
      }),
      environment: {
  ...
  private getCloudformationBuild(configs: CanaryConfig[]) {
    ...
    for (const config of configs) {
      synthCommands.push(
        `npm run cdk synth -- -c config=${config.env} -o dist`,
      );
    }
    ...
    return new codebuild.PipelineProject(this, "CFBuild", {
      buildSpec: codebuild.BuildSpec.fromObject({
        ...
        phases: {
          install: {
            commands: ["cd ops/aws/canary", "npm install"],
          },
          pre_build: {
  ...
  private getIntegrationTest(config: CanaryConfig) {
    return new codebuild.PipelineProject(
      this,
      "IntegrationTest" + config.envTitle,
      {
        buildSpec: codebuild.BuildSpec.fromObject({
          version: "0.2",
          env: {
          ...
          phases: {
            ...
            build: {
              commands: ["cd ops/aws/canary/lambda", "make integration-test"],
            },
          },
          reports: {
            IntegrationTest: {
              files: ["ops/aws/canary/lambda/tests.xml"],
              "discard-paths": "yes",
            },
          },
        }),
