...
For example, Rust users can specify the `wasm32-wasi` target to `rustup` and `cargo` to get programs compiled for WASI WebAssembly. See [the Rust example](../setting-up/workload-onboarding/Container/Rust-WASM.md) for more information on this.
...
See [the Rust example](../setting-up/workload-onboarding/Container/Rust-WASM.md) for a workload that leverages WebAssembly support.
