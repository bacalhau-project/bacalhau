...
func (scheduler *Libp2pScheduler) SetContext(ctx context.Context) {
	scheduler.Context = ctx
}

func (scheduler *Libp2pScheduler) GetContext() context.Context {
...
func (scheduler *Libp2pScheduler) Connect(peerConnect string) error {
	...
	//nolint
	scheduler.Host.Connect(scheduler.Context, *info)
...
func (scheduler *Libp2pScheduler) writeJobEvent(event *types.JobEvent) error {
	...
	return scheduler.JobEventTopic.Publish(scheduler.Context, msgBytes)
}

func (scheduler *Libp2pScheduler) readLoopJobEvents() {
	for {
		msg, err := scheduler.JobEventSubscription.Next(scheduler.Context)
		if err != nil {
			return
		}
