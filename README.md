...
# Filecoin Distributed Computation Framework

- Background:
    - __Definition__: A "large" dataset is one where the amount of data cannot be (reasonably) run on a single node.
    - __User Need__: A common need for using a large dataset is to do [embarrassingly parallel](https://en.wikipedia.org/wiki/Embarrassingly_parallel) compute jobs next to the data (e.g. data and compute are on the same physical/virtual device). Scenarios where this may need to occur include:
...
- Issues with the existing solutions:
    ...
    - However, running and maintaining such solutions is very challenging.
        - Computation APIs for these platforms are often difficult to understand and restricted to specific platforms (non-portable) or in new languages.
    ...
    - The market for such solutions is likely $100B+ (DataBricks (the company behind Spark), by itself, is worth $40B+).

**PUSH COMPUTE INTO GENE SEQUENCER**
**PIPE TO S3** 

- High level idea: 
    - IPFS is optimized for storing large datasets and excuting distributed jobs because each node (likely) holds only a subset of the data (enabling trivial parallel jobs) and (likely) has space compute (because mining, sealing, etc are very bursty leading to long idle times).
    - Protocol Labs should:
        - Enable submitting jobs to IPFS that can be sharded and run, in parallel, on many nodes.
        - Extend miners to allow shipping programs to nodes and run computation on the data available on each node.
        - Allow for returning the results of the jobs back to IPFS or any arbitrary endpoint.
    ...
    - At the end of the day, this will result in us enabling a distributed 'map reduce across all filecoin data' system
...
- Sample Scenario:
    - Process downscale large images to smaller images before use
...
## Items to still flesh out:
- Performance:
    - What performance task will we target (e.g. almost certainly not performant enough for ML requirements around weight transfers)
...
- Functionality extension:
    ...
    - Partnerships
        - Should we partner with Anorth/Glow who wants to develop a languages for certifiable data (IPLD) interpretation and transformation for use in decentralized systems.
...
**ACTION ITEMS**
- David to work with Dhash on Sponsored Grant
    - Make it work with Kubernetes (same pod should exist)
    - SRA - Sequence of short read sequences - NIH/NSF run out of money
