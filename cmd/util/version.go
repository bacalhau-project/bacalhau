...
func GetAllVersions(ctx context.Context) (Versions, error) {
	...
	if err != nil {
		return versions, errors.Wrap(err, "error getting user ID")
	}

	updateCheck, err := version.CheckForUpdate(ctx, versions.ClientVersion, versions.ServerVersion, clientID, userID)
	if err != nil {
