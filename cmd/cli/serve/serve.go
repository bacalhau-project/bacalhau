...
func serve(cmd *cobra.Command) error {
	...
	if isRequesterNode {
		...
		if len(key) == 0 || len(cert) == 0 {
			var caCert *util.CACertificate
			fmt.Printf("Checking for existing autogenerated keys and certificates")
			//olgibbons: Need to find a suitable means of storing keys/certs. Can potentially use UserIDKey for key.
			...
			caCertPath := filepath.Join(secretsPath, "ca_cert.pem")
			caKeyPath := filepath.Join(secretsPath, "ca_key.pem")

			if !util.FileExists(caCertPath) || !util.FileExists(caKeyPath) {
				fmt.Printf("Autogenerated CA certificates and keys not found. Generating them...")
				caCert, err = util.NewCACertificate(caCertPath, caKeyPath)
				if err != nil {
					return err
				}
			}
			...
			if !util.FileExists(serverCertPath) || !util.FileExists(serverKeyPath) {
				//olgibbons: must be a neater way of doing this?
				fmt.Printf("Autogenerating server certificates and keys...")
				ips := []net.IP{}
