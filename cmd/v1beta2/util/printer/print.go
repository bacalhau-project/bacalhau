...
var eventsWorthPrinting = map[v1beta2.JobStateType]eventStruct{
	v1beta2.JobStateNew:        {Message: "Creating job for submission", IsTerminal: false, PrintDownload: false, IsError: false},
	v1beta2.JobStateQueued:     {Message: "Job waiting to be scheduled", PrintDownload: true, IsTerminal: false, IsError: false},
	v1beta2.JobStateInProgress: {Message: "Job in progress", PrintDownload: true, IsTerminal: false, IsError: false},
	v1beta2.JobStateError:      {Message: "Error while executing the job", PrintDownload: false, IsTerminal: true, IsError: true},
	v1beta2.JobStateCancelled:  {Message: "Job canceled", PrintDownload: false, IsTerminal: true, IsError: false},
	v1beta2.JobStateCompleted:  {Message: "Job finished", PrintDownload: false, IsTerminal: true, IsError: false},
	v1beta2.JobStateCompletedPartially: {
...
func PrintJobExecution(
	...
	j *v1beta2.Job,
	cmd *cobra.Command,
	downloadSettings *flags2.DownloaderSettings,
	runtimeSettings *flags2.RunTimeSettings,
	...
	if runtimeSettings.Follow {
		...
		return util2.Logs(cmd, j.Metadata.ID, true, true)
	}
	...
	hasResults := slices.ContainsFunc(js.Executions, func(e v1beta2.ExecutionState) bool { return e.RunOutput != nil })
	if !quiet && hasResults {
	...
	if hasResults && runtimeSettings.AutoDownloadResults {
		if err := util2.DownloadResultsHandler(ctx, cmd, j.Metadata.ID, downloadSettings); err != nil {
			return err
		}
	}
	return nil
...
//
//nolint:gocyclo,funlen
func WaitForJobAndPrintResultsToUser(ctx context.Context, cmd *cobra.Command, j *v1beta2.Job, quiet bool) error {
...
To cancel the job, run:
   bacalhau cancel %s`, j.Metadata.ID)
	...
	// so far.
	printedEventsTracker := make(map[v1beta2.JobStateType]bool)
	for _, jobEventType := range v1beta2.JobStateTypes() {
	...
	signal.Notify(signalChan, util2.ShutdownSignals...)
	defer func() {
	...
	var lastEventState v1beta2.JobStateType
	for !cmdShuttingDown {
		// Get the job level history events that happened since the last one we saw
		jobEvents, err := util2.GetAPIClient(ctx).GetEvents(ctx, j.Metadata.ID, publicapi.EventFilterOptions{
		...
		for _, event := range jobEvents {
			...
			if event.Type != v1beta2.JobHistoryTypeJobLevel {
				continue
			}
			...
			if !quiet {
				...
				if !wasPrinted && eventsWorthPrinting[jet].Message != "" {
					...
					if event.JobState.New == v1beta2.JobStateQueued {
			...
			if event.JobState.New.IsTerminal() {
				if event.JobState.New != v1beta2.JobStateCompleted {
...
func summariseExecutions(state v1beta2.JobState) map[string][]string {
