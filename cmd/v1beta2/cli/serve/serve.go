...
type ServeOptions struct {
	...
	JobSelectionPolicy                    v1beta2.JobSelectionPolicy // How the node decides what jobs to run.
...
func NewServeOptions() *ServeOptions {
	return &ServeOptions{
		...
		JobSelectionPolicy:  v1beta2.NewDefaultJobSelectionPolicy(),
		LimitTotalCPU:       "",
		LimitTotalMemory:    "",
		LimitTotalGPU:       "",
		LimitJobCPU:         "",
...
func GetComputeConfig(OS *ServeOptions) node.ComputeConfig {
	return node.NewComputeConfigWith(node.ComputeConfigParams{
		...
		TotalResourceLimits: capacity.ParseResourceUsageConfig(v1beta2.ResourceUsageConfig{
			CPU:    OS.LimitTotalCPU,
			Memory: OS.LimitTotalMemory,
			GPU:    OS.LimitTotalGPU,
		}),
		JobResourceLimits: capacity.ParseResourceUsageConfig(v1beta2.ResourceUsageConfig{
			CPU:    OS.LimitJobCPU,
...
func serve(cmd *cobra.Command, OS *ServeOptions) error {
	ctx := cmd.Context()
	cm := util2.GetCleanupManager(ctx)
	...
	if util2.LoggingMode == logger.LogModeStation && standardNode.IsComputeNode() {
...
export BACALHAU_PEER_CONNECT=%s`, ipfsSwarmAddress, OS.HostAddress, util2.GetAPIPort(ctx), peerAddress)
