...
func (suite *CancelSuite) TestCancelTerminalJob() {
	...
	_, stdout, err := cmdtesting2.ExecuteTestCobraCommandWithStdinBytes(testdata.TaskDockerJson.Data, "create",
		"--api-host", suite.Host,
	...
	_, stdout, err = cmdtesting2.ExecuteTestCobraCommand("cancel",
		job.Metadata.ID,
		"--api-host", suite.Host,
		"--api-port", fmt.Sprint(suite.Port),
	)
...
func (suite *CancelSuite) TestCancelJob() {
	...
	_, stdout, err := cmdtesting2.ExecuteTestCobraCommandWithStdinBytes(testdata.JsonJobCancel.Data, "create",
		"--wait=false",
		"--api-host", suite.Host,
		"--api-port", fmt.Sprint(suite.Port),
	)
