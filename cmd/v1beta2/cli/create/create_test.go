...
func (s *CreateSuite) TestCreateGenericSubmitBetter() {
	...
	for _, tc := range tests {
		s.Run(tc.Name, func() {
			...
			_, out, err := cmdtesting2.ExecuteTestCobraCommandWithStdinBytes(tc.Fixture.Data, "create",
				"--api-host", s.Host,
...
func (s *CreateSuite) TestCreateFromStdin() {
	_, out, err := cmdtesting2.ExecuteTestCobraCommandWithStdinBytes(testdata.YamlJobNoop.Data, "create",
		"--api-host", s.Host,
		"--api-port", fmt.Sprint(s.Port),
	)
...
func (s *CreateSuite) TestCreateDontPanicOnEmptyFile() {
	...
	go func() {
		defer wg.Done()
		c, out, err := cmdtesting2.ExecuteTestCobraCommand("create", "./testdata/empty.yaml")
