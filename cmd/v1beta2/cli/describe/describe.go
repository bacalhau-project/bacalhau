...
func describe(cmd *cobra.Command, cmdArgs []string, OD *DescribeOptions) error {
	...
	j, foundJob, err := util2.GetWrappedAPIClient(ctx).Get(ctx, inputJobID)
	...
	if OD.IncludeEvents {
		jobEvents, err := util2.GetWrappedAPIClient(ctx).GetEvents(ctx, j.Job.Metadata.ID, publicapi.EventFilterOptions{})
		if err != nil {
