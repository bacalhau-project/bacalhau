...
`))
	...
	DefaultExcludedTags = []v1beta2.ExcludedTag{
		"canary",
	}
)
...
type ListOptions struct {
	...
	IncludeTags []v1beta2.IncludedTag // Only return jobs with these annotations
	ExcludeTags []v1beta2.ExcludedTag // Only return jobs without these annotations
...
func NewListOptions() *ListOptions {
	return &ListOptions{
		IDFilter:    "",
		IncludeTags: v1beta2.IncludeAny,
...
func NewCmd() *cobra.Command {
	...
	listCmd.PersistentFlags().Var(flags2.IncludedTagFlag(&OL.IncludeTags), "include-tag",
		`Only return jobs that have the passed tag in their annotations`)
	listCmd.PersistentFlags().Var(flags2.ExcludedTagFlag(&OL.ExcludeTags), "exclude-tag",
	...
	listCmd.PersistentFlags().AddFlagSet(flags2.OutputFormatFlags(&OL.OutputOpts))
...
var listColumns = []output.TableColumn[*v1beta2.JobWithInfo]{
	...
	},
	{
		...
		Value:        func(jwi *v1beta2.JobWithInfo) string { return job.ComputeStateSummary(jwi.State) },
	},
	{
...
func list(cmd *cobra.Command, OL *ListOptions) error {
	ctx := cmd.Context()
	jobs, err := util2.GetAPIClient(ctx).List(
		ctx,
		OL.IDFilter,
		OL.IncludeTags,
		OL.ExcludeTags,
