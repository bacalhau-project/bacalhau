...
func (s *DockerRunSuite) TestRun_GenericSubmit() {
	...
	for i, tc := range tests {
		s.Run(fmt.Sprintf("job%d", tc.numberOfJobs), func() {
			...
			randomUUID := uuid.New()
			_, out, err := cmdtesting2.ExecuteTestCobraCommand("docker", "run",
				"--api-host", s.Host,
...
func (s *DockerRunSuite) TestRun_GPURequests() {
	...
	for i, tc := range tests {
		func() {
			...
			_, out, submitErr := cmdtesting2.ExecuteTestCobraCommand(allArgs...)
	...
	for i, tc := range tests {
		...
		for _, tcids := range testCids {
			func() {
				...
				_, out, err := cmdtesting2.ExecuteTestCobraCommand(flagsArray...)
...
func (s *DockerRunSuite) TestRun_ExplodeVideos() {
	...
	_, _, submitErr := cmdtesting2.ExecuteTestCobraCommand(allArgs...)
...
func (s *DockerRunSuite) TestRun_Deterministic_Verifier() {
	...
	apiSubmitJob := func(
		...
		_, out, err := cmdtesting2.ExecuteTestCobraCommand(
			"docker", "run",
			"--api-host", host,
			"--api-port", port,
...
func (s *DockerRunSuite) TestRun_Timeout_DefaultValue() {
	...
	s.Require().Equal(j.Spec.Timeout, util2.DefaultTimeout.Seconds(), "Did not fall back to default timeout value")
}
