			...
			func() {
				...
				for i := 0; i < tc.numberOfAcceptNodes; i++ {
					for k := 0; k < n.numOfJobs; k++ {
						j := enginetesting.MakeDockerJob(s.T(), model.VerifierNoop, model.PublisherNoop, []string{"Entrypoint-Unique-Array", uuid.NewString()})
				...
				require.Equal(s.T(),
					enginetesting.DockerDecodeEngine(s.T(), submittedJob.Spec.Engine).Entrypoint[0],
					enginetesting.DockerDecodeEngine(s.T(), returnedJobDescription.Job.Spec.Engine).Entrypoint[0],
...
func (s *DescribeSuite) TestDescribeJobIncludeEvents() {
	...
	for _, tc := range tests {
		func() {
			...
			j := enginetesting.MakeDockerJob(s.T(), model.VerifierNoop, model.PublisherNoop, []string{"entrypoint"})
			job, err := s.client.Submit(ctx, j)
			require.NoError(s.T(), err)
			submittedJob = job // Default to the last job submitted, should be fine?
