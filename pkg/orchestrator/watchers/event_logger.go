...
func (e *ExecutionLogger) HandleEvent(ctx context.Context, event watcher.Event) error {
	...
	if event.ObjectType != jobstore.EventObjectExecutionUpsert {
		log.Ctx(ctx).Debug().Msgf("Skipping event of type %s", event.ObjectType)
		return nil
	}
	...
	if len(upsert.Events) > 0 {
		...
		eventLogger.Msgf("Execution %s generated %d new events", upsert.Current.ID, len(upsert.Events))
	}
