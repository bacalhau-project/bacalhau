...
type HeartbeatServer struct {
	...
	livenessMap       *concurrency.StripedMap[models.NodeLiveness]
	checkFrequency    time.Duration
...
func NewServer(params HeartbeatServerParams) (*HeartbeatServer, error) {
	...
	return &HeartbeatServer{
		...
		pqueue:            pqueue,
		livenessMap:       concurrency.NewStripedMap[models.NodeLiveness](0), // no particular stripe count for now
...
func (h *HeartbeatServer) markNodeAs(nodeID string, state models.NodeLiveness) {
...
func (h *HeartbeatServer) UpdateNodeInfo(state *models.NodeState) {
	if liveness, ok := h.livenessMap.Get(state.Info.NodeID); ok {
		...
		state.Liveness = models.NodeStates.DISCONNECTED
	}
}

// FilterNodeInfos will return only those NodeInfos that have the requested liveness
func (h *HeartbeatServer) FilterNodeInfos(nodeInfos []*models.NodeInfo, state models.NodeLiveness) []*models.NodeInfo {
