...
func SetupAPIServer(path string, cfg v2.Bacalhau) (*publicapi.Server, error) {
	authzPolicy, err := policy.FromPathOrDefault(cfg.Server.Auth.AccessPolicyPath, authz.AlwaysAllowPolicy)
	if err != nil {
		return nil, err
	}

	signingKey, err := loadUserIDKey(path)
	if err != nil {
		return nil, err
	}
	...
	serverParams := publicapi.ServerParams{
		...
		Authorizer: authz.NewPolicyAuthorizer(authzPolicy, &signingKey.PublicKey, cfg.Name),
