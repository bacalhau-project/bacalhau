...
func (s *LogStreamServer) Handle(stream network.Stream) {
	...
	log.Ctx(s.ctx).Debug().Msgf("Logserver finding executor for: %+v", execution.Job.Spec.EngineSpec.Type)

	jobSpec := execution.Job.Spec
	e, err := s.executors.Get(s.ctx, jobSpec.EngineSpec.Type)
	if err != nil {
		log.Ctx(s.ctx).Error().Msgf("failed to find executor for engine: %s", jobSpec.EngineSpec.Type)
		_ = stream.Reset()
		return
	}
