...
func (s *StoreSuite) TestUpdateExecutionConditionsVersionFail() {
	err := s.executionStore.CreateExecution(s.ctx, s.localExecutionState)
	s.Require().NoError(err)
...
func (s *StoreSuite) TestGetActiveExecutionSingle() {
	err := s.executionStore.CreateExecution(s.ctx, s.localExecutionState)
	s.NoError(err)
...
func (s *StoreSuite) TestGetActiveExecutionMultiple() {
...
func (s *StoreSuite) TestGetActiveExecutionDoesntExist() {
	_, err := store.GetActiveExecution(s.ctx, s.executionStore, s.execution.JobID)
	s.ErrorAs(err, &store.ErrExecutionsNotFoundForJob{})
}
