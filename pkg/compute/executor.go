...
func (e *BaseExecutor) Run(ctx context.Context, execution store.Execution) (err error) {
	...
	jobExecutor, err := e.executors.Get(ctx, execution.Job.Spec.EngineSpec.Type)
	if err != nil {
		err = fmt.Errorf("failed to get executor %s: %w", execution.Job.Spec.EngineSpec.Type, err)
		return
	}
