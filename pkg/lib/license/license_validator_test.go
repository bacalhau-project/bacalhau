...
func TestNewOfflineLicenseValidator(t *testing.T) {
	// Create validator using offline public keys
	// Those are actual officially signed keys for testing
	...
	assert.Equal(t, "Bacalhau", claims.Product)
	assert.Equal(t, "v1", claims.LicenseVersion)
	assert.Equal(t, "standard", claims.LicenseType)
	assert.Equal(t, "e66d1f3a-a8d8-4d57-8f14-00722844afe2", claims.LicenseID)
	assert.Equal(t, "test-customer-id-123", claims.CustomerID)
	assert.Equal(t, "test-customer-id-123", claims.Subject)
	assert.Equal(t, "https://expanso.io/", claims.Issuer)
	assert.Equal(t, "e66d1f3a-a8d8-4d57-8f14-00722844afe2", claims.ID) // jti claim
	assert.Equal(t, map[string]string{"max_nodes": "1"}, claims.Capabilities)
	assert.Equal(t, map[string]string{}, claims.Metadata)

	// Verify timestamps
	assert.Equal(t, int64(1736881638), claims.IssuedAt.Unix())
	assert.Equal(t, int64(2384881638), claims.ExpiresAt.Unix())
}
