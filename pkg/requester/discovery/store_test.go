...
func (s *StoreNodeDiscovererSuite) TestFindNodes() {
	...
	job := model.Job{}
	job.Spec.EngineDeprecated = model.EngineDocker
	job.Spec.EngineSpec = model.NewDockerEngineSpec("", nil, nil, "")
	...
	job.Spec.EngineDeprecated = model.EngineNoop
	job.Spec.EngineSpec = model.EngineSpec{Type: model.EngineNoop.String()}
	peerIDs, err = s.discoverer.FindNodes(context.Background(), job)
	s.NoError(err)
	s.Empty(peerIDs)
}
