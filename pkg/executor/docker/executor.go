...
import (
	...
	utilContext "github.com/bacalhau-project/bacalhau/pkg/util/context"
...
func (e *Executor) RunShard(
 ...
 or registry needing authorization`, shard.Job.Spec.Docker.Image)
	...
	detachedContext, cancel := context.WithTimeout(utilContext.NewDetachedContext(ctx), 3*time.Second)
	defer cancel()
...
func (e *Executor) cleanupJob(ctx context.Context, shard model.JobShard) {
	...
	separateCtx, cancel := context.WithTimeout(utilContext.NewDetachedContext(ctx), 1*time.Minute)
	defer cancel()
...
func (e *Executor) cleanupAll(ctx context.Context) error {
	...
	safeCtx := utilContext.NewDetachedContext(ctx)
