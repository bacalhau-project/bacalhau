...
func (s *InputLocalityStrategy) ShouldBid(
	...
	// are local to us
	e, err := s.executors.Get(ctx, request.Job.Spec.EngineSpec.Type)
	if err != nil {
		return bidstrategy.BidStrategyResponse{}, fmt.Errorf("InputLocalityStrategy: failed to get executor %s: %w", request.Job.Spec.EngineSpec.Type, err)
	}
