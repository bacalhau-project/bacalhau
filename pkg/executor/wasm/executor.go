...
func (e *Executor) Start(ctx context.Context, request *executor.RunCommandRequest) error {
	...
	if handler, found := e.handlers.Get(request.ExecutionID); found {
		if handler.active() {
			return NewWasmExecutorError(executor.ExecutionAlreadyStarted, fmt.Sprintf("starting execution (%s)", request.ExecutionID))
		} else {
			return NewWasmExecutorError(executor.ExecutionAlreadyComplete, fmt.Sprintf("starting execution (%s)", request.ExecutionID))
		}
	}
...
func (e *Executor) Wait(ctx context.Context, executionID string) (<-chan *models.RunCommandResult, <-chan error) {
	...
	if !found {
		errCh <- NewWasmExecutorError(executor.ExecutionNotFound, fmt.Sprintf("waiting on execution (%s)", executionID))
		return outCh, errCh
	}
...
func (e *Executor) Cancel(ctx context.Context, executionID string) error {
	handler, found := e.handlers.Get(executionID)
	if !found {
		return NewWasmExecutorError(executor.ExecutionNotFound, fmt.Sprintf("canceling execution (%s)", executionID))
	}
...
func (e *Executor) GetLogStream(ctx context.Context, request executor.LogStreamRequest) (io.ReadCloser, error) {
	...
	if !found {
		return nil, NewWasmExecutorError(executor.ExecutionNotFound, fmt.Sprintf("getting outputs for execution (%s)", request.ExecutionID))
	}
