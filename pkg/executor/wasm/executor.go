...
func (e *Executor) Run(ctx context.Context, executionID string, job model.Job, jobResultsDir string) (*model.RunCommandResult, error) {
	...
	wasmEngineSpec, err := job.Spec.EngineSpec.AsWasmSpec()
	if err != nil {
		return nil, err
	}

	args := append([]string{wasmEngineSpec.EntryModule.Name}, wasmEngineSpec.Parameters...)
	...
	keys := maps.Keys(job.Spec.EnvironmentVariables)
	sort.Strings(keys)
	for _, key := range keys {
		// Make sure we add the environment variables in a consistent order
		config = config.WithEnv(key, job.Spec.EnvironmentVariables[key])
	}

	// TODO Currently unused see #880: Other WASM modules whose exports will be available as imports
	// Load and instantiate imported modules
	loader := NewModuleLoader(engine, config, e.StorageProvider)
	for _, importModule := range wasmEngineSpec.ImportModules {
		_, ierr := loader.InstantiateRemoteModule(ctx, importModule)
		err = multierr.Append(err, ierr)
	}

	// Load and instantiate the entry module.
	instance, err := loader.InstantiateRemoteModule(ctx, wasmEngineSpec.EntryModule)
	if err != nil {
		return executor.FailResult(err)
	}
	...
	log.Ctx(ctx).Debug().
		Str("entryPoint", wasmEngineSpec.EntryPoint).
		Msg("Running WASM job")
	entryFunc := instance.ExportedFunction(wasmEngineSpec.EntryPoint)
	exitCode := -1
