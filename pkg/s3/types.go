...
func (c SourceSpec) Validate() error {
	if c.Bucket == "" {
		return NewBadS3RequestError("invalid s3 storage params: bucket cannot be empty")
	}
	return nil
}
...
func (c PreSignedResultSpec) Validate() error {
	if c.PreSignedURL == "" {
		return NewBadS3RequestError("invalid s3 signed storage params: signed url cannot be empty")
	}
...
func DecodeSourceSpec(spec *models.SpecConfig) (SourceSpec, error) {
	if !spec.IsType(models.StorageSourceS3) {
		return SourceSpec{}, NewBadS3RequestError("invalid storage source type. expected " + models.StorageSourceS3 + ", but received: " + spec.Type)
	}
	inputParams := spec.Params
	if inputParams == nil {
		return SourceSpec{}, NewBadS3RequestError("invalid storage source params. cannot be nil")
	}
...
func DecodePreSignedResultSpec(spec *models.SpecConfig) (PreSignedResultSpec, error) {
	if !spec.IsType(models.StorageSourceS3PreSigned) {
		return PreSignedResultSpec{}, NewBadS3RequestError(
			"invalid storage source type. expected " + models.StorageSourceS3PreSigned + ", but received: " + spec.Type)
	}

	inputParams := spec.Params
	if inputParams == nil {
		return PreSignedResultSpec{}, NewBadS3RequestError("invalid signed result params. cannot be nil")
	}
...
func (c PublisherSpec) Validate() error {
	if c.Bucket == "" {
		return NewBadS3RequestError("invalid s3 params. bucket cannot be empty")
	}
	if c.Key == "" {
		return NewBadS3RequestError("invalid s3 params. key cannot be empty")
	}
	return nil
}
...
func DecodePublisherSpec(spec *models.SpecConfig) (PublisherSpec, error) {
	if !spec.IsType(models.PublisherS3) {
		return PublisherSpec{}, NewBadS3RequestError(
			fmt.Sprintf("invalid publisher type. expected %s, but received: %s",
				models.PublisherS3, spec.Type))
	}
	inputParams := spec.Params
	if inputParams == nil {
		return PublisherSpec{}, NewBadS3RequestError("invalid publisher params. cannot be nil")
	}
