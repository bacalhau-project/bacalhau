...
// TODO @enricorotundo: this is a temporary solution to have a hashable value
...
// TODO @enricorotundo: this is a temporary solution to have a hashable value
// func hashStorageSpec(spec *StorageSpec) string {
// 	s := ""

// 	s += spec.StorageSource.String()
// 	s += spec.Name
// 	s += spec.CID
// 	s += spec.URL
// 	s += spec.Path
// 	s += stringifyMap(spec.Metadata)

// 	return s
// }

// TODO @enricorotundo: this is a temporary solution to have a hashable value
func HashableValue(j *JobCreatePayload) ([]byte, error) {
	s := ""
	s += j.ClientID
	s += j.APIVersion

	// // Spec
	// s += j.Spec.Engine.String()
	// s += j.Spec.Verifier.String()
	// s += j.Spec.Publisher.String()

	// // Spec.Docker
	// s += j.Spec.Docker.Image
	// s += strings.Join(j.Spec.Docker.Entrypoint, "")
	// s += strings.Join(j.Spec.Docker.EnvironmentVariables, "")
	// s += j.Spec.Docker.WorkingDirectory

	// // Spec.Language
	// s += j.Spec.Language.Language
	// s += j.Spec.Language.LanguageVersion
	// s += strconv.FormatBool(j.Spec.Language.Deterministic) // small case true/false
	// s += j.Spec.Language.Context.StorageSource.String()
	// s += j.Spec.Language.Context.Name
	// s += j.Spec.Language.Context.CID
	// s += j.Spec.Language.Context.URL
	// s += j.Spec.Language.Context.Path
	// s += stringifyMap(j.Spec.Language.Context.Metadata) // sorted map keys in increasing order

	// // Spec.Wasm
	// s += j.Spec.Wasm.EntryPoint
	// s += strings.Join(j.Spec.Wasm.Parameters, "")
	// s += stringifyMap(j.Spec.Wasm.EnvironmentVariables) // sorted map keys in increasing order
	// for _, importModule := range j.Spec.Wasm.ImportModules {
	// 	s += hashStorageSpec(&importModule)
	// }

	// // Spec.Resources
	// s += j.Spec.Resources.CPU
	// s += j.Spec.Resources.Memory
	// s += j.Spec.Resources.Disk
	// s += j.Spec.Resources.GPU

	// // Spec.Timeout
	// s += fmt.Sprintf("%f", j.Spec.Timeout)

	// // Spec.Inputs
	// for _, storageSpec := range j.Spec.Inputs {
	// 	s += hashStorageSpec(&storageSpec)
	// }

	// // Spec.Contexts
	// for _, storageSpec := range j.Spec.Contexts {
	// 	s += hashStorageSpec(&storageSpec)
	// }

	// // Spec.Outputs
	// for _, storageSpec := range j.Spec.Outputs {
	// 	s += hashStorageSpec(&storageSpec)
	// }

	// // Spec.Annotations
	// s += strings.Join(j.Spec.Annotations, "")

	// // Spec.Sharding
	// s += j.Spec.Sharding.GlobPattern
	// s += fmt.Sprintf("%d", j.Spec.Sharding.BatchSize)
	// s += j.Spec.Sharding.BasePath

	// // Spec.DoNotTrack
	// s += strconv.FormatBool(j.Spec.DoNotTrack) // small case true/false
	// // Spec.ExecutionPlan
	// s += fmt.Sprintf("%d", j.Spec.ExecutionPlan.TotalShards)

	// // Spec.Deal
	// s += fmt.Sprintf("%d", j.Spec.Deal.Concurrency)
	// s += fmt.Sprintf("%d", j.Spec.Deal.Confidence)
	// s += fmt.Sprintf("%d", j.Spec.Deal.MinBids)
