...
func (driver *StorageProvider) HasStorageLocally(_ context.Context, volume models.InputSource) (bool, error) {
	...
	if _, err := os.Stat(source.SourcePath); errors.Is(err, os.ErrNotExist) {
		...
		// In order to be able to create it, ReadWrite must be true.
		return source.ReadWrite, nil
	}
	return true, nil
}
...
func (driver *StorageProvider) GetVolumeSize(_ context.Context, _ *models.Execution, volume models.InputSource) (uint64, error) {
	...
	if _, err := os.Stat(source.SourcePath); errors.Is(err, os.ErrNotExist) {
		...
		// we only know that it's going to be an empty file/directory
		if source.ReadWrite {
			return 0, nil
		}
		// if the volume does not exist and we can't create it, return an error
		return 0, bacerrors.Newf("volume %s does not exist and read-write access is not allowed", source.SourcePath).
...
func (driver *StorageProvider) createVolumeIfNotExists(source Source) error {
	...
	var inferedCreateStrategy CreateStrategy
	if source.CreateAs == Infer {
		inferedCreateStrategy = InferCreateStrategyFromPath(source.SourcePath)
	} else {
		inferedCreateStrategy = source.CreateAs
	}

	switch inferedCreateStrategy {
	case NoCreate:
...
func createEmptyFile(filePath string) error {
	...
	err = os.Chmod(filePath, util.OS_USER_RWX)
	if err != nil {
		return bacerrors.Wrapf(err, "could not set permissions for source file at %s", filePath)
	}
	return nil
}
