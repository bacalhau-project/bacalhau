...
func (s *InMemoryNodeInfoStoreSuite) Test_Get() {
	...
	nodeInfo2 := generateNodeInfo("node2", wasm.EngineType)
...
func (s *InMemoryNodeInfoStoreSuite) Test_ListForEngine() {
	...
	nodeInfo1 := generateNodeInfo("node1", docker.EngineType)
	nodeInfo2 := generateNodeInfo("node2", wasm.EngineType)
	nodeInfo3 := generateNodeInfo("node3", docker.EngineType, wasm.EngineType)
	s.NoError(s.store.Add(ctx, nodeInfo1))
	s.NoError(s.store.Add(ctx, nodeInfo2))
	s.NoError(s.store.Add(ctx, nodeInfo3))

	dockerNodes, err := s.store.ListForEngine(ctx, docker.EngineType)
	s.NoError(err)
	s.ElementsMatch([]model.NodeInfo{nodeInfo1, nodeInfo3}, dockerNodes)

	wasmNodes, err := s.store.ListForEngine(ctx, wasm.EngineType)
	s.NoError(err)
...
func (s *InMemoryNodeInfoStoreSuite) Test_Replace() {
	...
	nodeInfo2 := generateNodeInfo("node1", wasm.EngineType)
