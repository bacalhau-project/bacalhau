...
const (
	...
	BucketJobVersions    = "versions"  // New bucket for job versions
	BucketJobsNames      = "job_names" // job-name -> Job id
...
func (b *BoltJobStore) getExecutions(
	...
	err = bkt.ForEach(func(_ []byte, v []byte) error {
		...
		if b.filterJobExecutionItem(es, options) {
			es.Job = job
...
func (b *BoltJobStore) filterJobExecutionItem(item models.Execution, query jobstore.GetExecutionsOptions) bool {
...
func (b *BoltJobStore) createJob(
	...
	jobNameKey := fmt.Sprintf("%s.j.%s.n", job.Name, job.Namespace)
	jobNamesBucket, err := NewBucketPath(BucketJobsNames).Get(tx, true)
	if err != nil {
		return NewBoltDBError(err)
	}
...
func (b *BoltJobStore) deleteJob(
	...
	jobNameKey := fmt.Sprintf("%s.j.%s.n", job.Name, job.Namespace)
	jobNamesBucket, err := NewBucketPath(BucketJobsNames).Get(tx, false)
	if err != nil {
		return NewBoltDBError(err)
	}
...
func (b *BoltJobStore) updateJob(
	...
	jobNameKey := fmt.Sprintf("%s.j.%s.n", existingJob.Name, existingJob.Namespace)
	jobNamesBucket, err := NewBucketPath(BucketJobsNames).Get(tx, true)
	if err != nil {
		return NewBoltDBError(err)
	}
...
func (b *BoltJobStore) updateJobState(ctx context.Context, tx *bolt.Tx, recorder *telemetry.MetricRecorder,
	...
	job.Revision++
	// Only increment Version when job specification changes
	// For state changes, we generally don't increment Version
	job.ModifyTime = b.clock.Now().UTC().UnixNano()
...
func (b *BoltJobStore) getJobByName(
	...
	jobNameKey := fmt.Sprintf("%s.j.%s.n", name, namespace)
