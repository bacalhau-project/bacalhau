...
func NewPublisherProvider(
	...
	mappedProvider := provider.NewMappedProvider(map[string]publisher.Publisher{
	...
	// We will only allow IPFS storage when we have an IPFS client
	if cl != nil {
		...
		mappedProvider.Add(models.PublisherIPFS, tracing.Wrap(ipfsPublisher))
	}
