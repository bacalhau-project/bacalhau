...
func ValidatePath(path string) error {
	...
	if strings.Contains(path, "$") {
		return bacerrors.New(fmt.Sprintf("data dir path %q contains a '$' character", path)).
	...
	if !filepath.IsAbs(path) {
		return bacerrors.New(fmt.Sprintf("data dir path %q is not an absolute path", path)).
	...
	for name, flagList := range flags {
		if cfgValue, exists := cfgValues[name]; exists {
			for _, flag := range flagList {
				if flag.Changed {
					return bacerrors.New(fmt.Sprintf("flag: --%s and config flag key %q cannot both be provided. Only one may be used",
						flag.Name, name)).
