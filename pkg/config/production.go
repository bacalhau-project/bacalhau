...
func init() {
	types.Default = Production()
}

func Production() types.BacalhauConfig {
	return types.BacalhauConfig{
...
func ProductionComputeConfig() types.ComputeConfig {
	return types.ComputeConfig{
		...
		JobTimeouts: types.JobTimeoutConfig{
			...
			MaxJobExecutionTimeout:                types.Duration(60 * time.Minute),
...
func ProductionRequesterConfig() types.RequesterConfig {
	return types.RequesterConfig{
