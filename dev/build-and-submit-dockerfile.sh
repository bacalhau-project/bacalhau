...
ABBREVIATED_GO_VERSION=$(echo $GO_VERSION | cut -d '.' -f 1,2)
export ABBREVIATED_GO_VERSION

# Build docker image with tag of latest tagged version
docker buildx build --platform linux/amd64,linux/arm64 --build-arg "GO_VERSION=${ABBREVIATED_GO_VERSION}" --push -t "docker.io/bacalhauproject/bacalhau-devcontainer:${latest_tag}" .