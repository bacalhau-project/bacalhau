...
import (
	...
	legacyExecutor "github.com/bacalhau-project/bacalhau/pkg/executor"
...
type executionHandler struct {
	...
	inputs []*executor.PreparedStorage
...
func (h *executionHandler) run(ctx context.Context) {
	...
	for _, importModule := range h.arguments.ImportModules {
		...
		if err != nil {
			h.logger.Warn().
				...
				Int32("volume_type", importModule.Volume.Type).
			...
			reason := fmt.Errorf("failed to instantiate import module (%s): %w", importModule.InputSource.Source["Type"], err).Error()
			h.result = &models.RunCommandResult{ErrorMsg: reason, ExitCode: -1}
	...
	if err != nil {
		h.logger.Warn().
			...
			Int32("volume_type", entryModule.Volume.Type).
		...
		reason := fmt.Errorf("failed to instantiate entry module module (%s): %w", entryModule.InputSource.Source["Type"], err).Error()
		h.result =
	...
	limits := ToLegacyOutputLimits(h.limits)
	h.result = legacyExecutor.WriteJobResults(h.resultsDir, stdoutReader, stderrReader, int(exitCode), wasmErr, limits)
}

func ToLegacyOutputLimits(limits *executor.OutputLimits) legacyExecutor.OutputLimits {
	return legacyExecutor.OutputLimits{
		MaxStdoutFileLength:   datasize.ByteSize(limits.MaxStdoutFileLength),
		MaxStdoutReturnLength: datasize.ByteSize(limits.MaxStdoutReturnLength),
		MaxStderrFileLength:   datasize.ByteSize(limits.MaxStderrFileLength),
		MaxStderrReturnLength: datasize.ByteSize(limits.MaxStderrReturnLength),
	}
}

type LegacyOutputLimits struct {
