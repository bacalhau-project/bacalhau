...
func (c EngineSpec) ToArguments(entryModule *executor.PreparedStorage, importModules ...executor.PreparedStorage) EngineArguments {
...
type EngineArguments struct {
	...
	Parameters           []string
	EnvironmentVariables map[string]string
	EntryModule          *executor.PreparedStorage
	ImportModules        []executor.PreparedStorage
}

func (c *EngineArguments) Validate() error {
	if c.EntryModule.InputSource == nil {
...
func DecodeArguments(spec map[string]string) (*EngineArguments, error) {

	if spec["Type"] != "wasm" {
		return nil, errors.New("invalid wasm engine type. expected 'wasm' but received: " + spec["Type"])
	}

	inputParams := spec["Params"]
	if inputParams == "" {
		return nil, errors.New("invalid wasm engine params. cannot be empty")
	}
