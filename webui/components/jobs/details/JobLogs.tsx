...
import { AlertCircle, Play, RefreshCcw, StopCircle, CheckCircle } from 'lucide-react';
...
const JobLogs = ({ jobId }: { jobId: string | undefined }) => {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    ws.onmessage = (event) => {
      ...
      try {
        ...
        if (message.value && message.value.Line) {
          setLogs(prevLogs => [...prevLogs, { type: message.value.Type, content: message.value.Line }]);
    ...
    ws.onclose = (event) => {
      ...
      wsRef.current = null;
      if (event.code !== 1000) {
        setError(`Connection closed unexpectedly (${event.code}). Please try reconnecting.`);
      }
    };
    ...
    <Card className="bg-gray-900 text-green-400 h-full">
      <CardHeader className="p-5">
        <CardTitle className="flex justify-between items-center">
          ...
          {isStreaming ? (
            <Button onClick={handleStopStreaming} variant="destructive" size="sm">
              <StopCircle className="mr-2 h-4 w-4" />
              Stop Streaming
            </Button>
          ) : (
            <Button onClick={handleStartStreaming} variant="ghost" className="bg-green-400 text-gray-900" size="sm">
      ...
      <CardContent>
        {error ? (
          <div className="flex items-center text-red-500 mb-4">
            ...
            <span>{error}</span>
            <Button onClick={handleStartStreaming} variant="ghost" size="sm" className="ml-2">
