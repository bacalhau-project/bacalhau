...
const formatResource = (available: number | undefined, total: number | undefined, unit: string): string => {
...
const ResourceBar: React.FC<{
  ...
  return (
    <div className={`flex items-center space-x-2 ${variant === 'large' ? 'mb-5' : variant === 'compact' ? 'mb-0.5' : 'mb-1'}`}>
      {variant !== 'compact' && (
        <div className={`${variant === 'large' ? 'w-24' : 'w-20 text-sm '} flex items-center`}>
          <span className="mr-2">{icon}</span>
          {label}
        </div>
      )}
      <div className={`flex-grow bg-gray-100 rounded-md ${variant === 'large' ? 'h-4' : variant === 'compact' ? 'h-1' : 'h-2'}`}>
        <div
          className={`bg-blue-500 rounded-md ${variant === 'large' ? 'h-4' : variant === 'compact' ? 'h-1' : 'h-2'}`}
          style={{ width: `${percentage}%` }}
        ></div>
      </div>
      <div className={`${variant === 'large' ? 'w-32' : variant === 'compact' ? 'w-16 text-sm ' : 'w-24 text-sm '} text-right`}>
...
export const NodeResources: React.FC<NodeResourcesProps> = ({ node, variant = 'default' }) => {
  ...
  if (!availableCapacity || !maxCapacity) {
    return <span className="text-sm text-gray-500">No resource information available</span>
  }
